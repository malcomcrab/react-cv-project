...
function SkillEditor({ skillEditData, setSkillsArray, skillsArray }) {
  ...
  function createNewFormObject(event) {
    ...
    let updatedSkillFormObject = Object.fromEntries(
      updatedSkillEntry.entries(),
    );
  ...
  function replaceEntry(newFormData) {
    let filteredArray = skillsArray.filter(
      (item) => item.id != skillEditData.id,
    );
  ...
  return (
    <>
      <form
        key={skillEditData.id}
        value={skillEditData.id}
        onSubmit={handleSkillEditSubmit}
      >
        <fieldset>
